...
async def create_policy_query(
    ...
    answer = await policy_service.query_policy(query=user_query,model=model)
...
async def create_research_plan(
    ...
    """Create a distributed research plan and answer the user's question.

    This node builds and executes a multi-tool workflow that performs
    task decomposition and queries the local knowledge base.

    Args:
        state (AgentState): Current agent state, including conversation history.
        config (RunnableConfig): Configuration used for workflow execution.

    Returns:
        Dict[str, List[BaseMessage]]:
            Assistant response generated by the multi-tool workflow.
    """
    logger.info("------execute local knowledge base query------")

    # # Use the language model to generate a multi-hop or parallel query plan.
    if settings.AGENT_SERVICE == ServiceType.OPENAI:
    ...
    # 1. Connect to the Neo4j graph database using the configured connection.
    try:
        neo4j_graph = get_neo4j_graph()
        logger.info("success to get Neo4j graph database connection")
    except Exception as e:
    ...
    # 2. Create a custom retriever. It retrieves Cypher examples based on the graph schema to guide the LLM in generating more accurate Cypher queries.
    cypher_retriever = NorthwindCypherRetriever()

    # Step 3. Define the available tool schemas.  
    ...
    # Load predefined Cypher queries for common e-commerce scenarios.
    ...
    # Prepare the workflow input state.
