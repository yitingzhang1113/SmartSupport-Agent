...
# Get logger
logger = get_logger(service="PredefinedCypher_tools")
...
def create_predefined_cypher_node(
    ...
    async def predefined_cypher(
        ...
        cypher_name = params.get("query", "")  
        # statement = predefined_cypher_dict.get(params.get("query"))
        statement = predefined_cypher_dict.get(cypher_name)
        query_params = params.get("parameters") or {}
        ...
        if statement is not None:
            ...
            try:
                records = graph.query(query=statement,params=query_params)
            except Exception as exc:
                logger.exception("Failed to execute predefined Cypher.")
                errors.append(str(exc))
                records = []
        else:
            errors.append(f"Unable to find the specified Cypher statement: {statement_name}")
            records = list()
