...
def create_multi_tool_workflow(
    ...
    CompiledStateGraph
        The workflow.
    """
    # 1. Create guardrails node
    # The guardrails node determines whether the incoming question falls within the scope of the search (for example, whether it is related to the e-commerce (products of our own)). If not, it provides a default message and the workflow routes to the final answer generation.
    guardrails = create_guardrails_node(
        llm=llm, graph=graph, scope_description=scope_description
    )

    # 2. If using guardrails, the task will be decomposed based on the user's question.
    planner = create_planner_node(llm=llm)

    # 3. Create the "cypher_query" node to generate Cypher query statements based on the user's questions
    ...
    # Select the tool node and choose the appropriate tool based on the user's question
